<?php
class ControllerModuleServicesActiveInactive extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('module/services_active_inactive');

        $this->document->setTitle($this->language->get('heading_title'));

        // $this->load->model('setting/setting');

        if (isset($this->request->get['extension']) && ($this->request->get['extension'] == 'services')) {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/services_active_inactive', 'token=' . $this->session->data['token'], 'SSL')
        );

        $data['active'] = $this->url->link('module/services_active_inactive', 'token=' . $this->session->data['token'] . '&extension=services&action=add', 'SSL');
        $data['inactive'] = $this->url->link('module/services_active_inactive', 'token=' . $this->session->data['token'] . '&extension=services&action=remove', 'SSL');
        ...
        $this->response->setOutput($this->load->view('module/services_active_inactive.tpl', $data));
    }
