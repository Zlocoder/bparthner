<?php
class ControllerModuleServicesTabs extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('module/services_tabs');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_extension_module->addModule('services_tabs', $this->request->post);
            } else {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/services_tabs', 'token=' . $this->session->data['token'], 'SSL')
        );

        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('module/services_tabs', 'token=' . $this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('module/services_tabs', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
        ...
        $this->response->setOutput($this->load->view('module/services_tabs.tpl', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'module/services_tabs')) {
