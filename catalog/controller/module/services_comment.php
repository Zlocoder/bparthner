<?php
class ControllerModuleServicesComment extends Controller {
    ...
    public function index($settings) {
        ...
        $this->load->language('module/services_comment');
        ...
        $comments = $this->model_services_comment->comments(array("bc.comment_post_ID"=>"='".$pid."'", "bc.comment_approve"=>"='publish'"), "bc.comment_ID DESC", ($page - 1) * $limit, $limit);
        $totla_comment = $this->model_services_comment->total_comment(array("comment_post_ID" => "='".$pid."'"));
        ...
        $post_info = $this->model_services_post->getPost(array("ID"=>"='".$pid."'"));
        ...
        $pagination->url = $this->url->link('services/single&pid='.$pid,'&page={page}#services-comment');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/services_comment.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/module/services_comment.tpl', $data);
        } else {
            return $this->load->view('default/template/module/services_comment.tpl', $this->data);
        }
    }
    ...
    public function submit() {
        ...
        if(!isset($this->request->get['pid'])) {
            $this->response->redirect($this->url->link('services/home'));
        }
        ...
        $services_setting = $this->model_services_setting->settings();
        $config = setting($services_setting);
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                $this->load->model('module/services_comment');
                $this->model_module_services_comment->addComment($pid, $this->request->post,$config);
                if($this->request->post['ajax']) {
                ...
                $this->model_module_services_comment->increaseComment($pid);
            }
            ...
            if($this->request->post['ajax']) {
                ...
                $this->response->redirect($this->url->link('services/single&pid='.$this->request->get['pid'].'#commentAlert'));
            }
        } // $_POST
    }
}