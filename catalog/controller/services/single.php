<?php
class ControllerServicesSingle extends Controller {
	public function index() {
		$this->load->language('services/single');
		...
		if(isset($this->request->get['pid'])) {
			...
			$post = $this->model_services_post->getPost(array("p.ID"=>"='".$this->request->get['pid']."'"));
			...
			if($post) {
				...
				$data['breadcrumbs'][] = array(
					...
					'href' => $this->url->link('services/single?pid=' . $post['title'], '', 'SSL')
				);

				$this->model_services_post->increaseView($pid);	

				$thumb_size = explode('x', $services_config['related_post_image_size']);
				...
				if($post['tag']) {
					...
					for ($i=0; $i < count($tags); $i++) { 
						...
						$related_posts = $this->model_services_post->getPosts('',array("pd.tag" => " LIKE '%".$tags[$i]."%'","p.ID" => " NOT IN ('".$post['ID']."',".$post_ids.")"),'',"p.date_modified ASC",0,1);
						if($related_posts) {
							$post_thumb = array();
				...
				$related_product_id =  $this->model_services_post->related_product(array("post_id"=>"='".$post['ID']."'"));
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/services/single.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/services/single.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/services/single.tpl', $data));
			}
